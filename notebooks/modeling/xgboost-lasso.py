...
def munge(df):
    ...
    #all_df["BsmtQual"] = df["BsmtQual"].map(qual_dict).astype(float)
    all_df["BsmtQual"] = np.nan_to_num(df["BsmtQual"].map(qual_dict)).astype(int)
    
    #all_df["BsmtCond"] = df["BsmtCond"].map(qual_dict).astype(int)
    all_df["BsmtCond"] = np.nan_to_num(df["BsmtCond"].map(qual_dict)).astype(int)

    all_df["HeatingQC"] = np.nan_to_num(df["HeatingQC"].map(qual_dict)).astype(int)
    all_df["KitchenQual"] = np.nan_to_num(df["KitchenQual"].map(qual_dict)).astype(int)
    
    #all_df["FireplaceQu"] = df["FireplaceQu"].map(qual_dict).astype(int)
    all_df["FireplaceQu"] = np.nan_to_num(df["FireplaceQu"].map(qual_dict)).astype(int)
    
    #all_df["GarageQual"] = df["GarageQual"].map(qual_dict).astype(float)
    all_df["GarageQual"] = np.nan_to_num(df["GarageQual"].map(qual_dict)).astype(int)
    
    #all_df["GarageCond"] = df["GarageCond"].map(qual_dict).astype(int)
    all_df["GarageCond"] = np.nan_to_num(df["GarageCond"].map(qual_dict)).astype(int)

    exposure_dict = {None: 0, "No": 1, "Mn": 2, "Av": 3, "Gd": 4}
    #all_df["BsmtExposure"] = df["BsmtExposure"].map(exposure_dict).astype(int)
    all_df["BsmtExposure"] = np.nan_to_num(df["BsmtExposure"].map(exposure_dict)).astype(int)

    
    bsmt_fin_dict = {None: 0, "Unf": 1, "LwQ": 2, "Rec": 3, "BLQ": 4, "ALQ": 5, "GLQ": 6}
    all_df["BsmtFinType1"] = np.nan_to_num(df["BsmtFinType1"].map(bsmt_fin_dict)).astype(int)
    all_df["BsmtFinType2"] = np.nan_to_num(df["BsmtFinType2"].map(bsmt_fin_dict)).astype(int)

    all_df["Functional"] = np.nan_to_num(df["Functional"].map(
        {None: 0, "Sal": 1, "Sev": 2, "Maj2": 3, "Maj1": 4, 
         "Mod": 5, "Min2": 6, "Min1": 7, "Typ": 8})).astype(int)

    all_df["GarageFinish"] = np.nan_to_num(df["GarageFinish"].map(
        {None: 0, "Unf": 1, "RFn": 2, "Fin": 3})).astype(int)

    all_df["Fence"] = np.nan_to_num(df["Fence"].map(
    ...
    all_df["PoolQC"] = np.nan_to_num(df["PoolQC"].map(qual_dict)).astype(int)
